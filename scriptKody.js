$(document).ready(function () {
  ...
  $("#getCityButton").click(function () {
    ...
    var response = $.ajax({
      url: `api/postalCodes.php?type=city&city=${city}`,
      dataType: "json",
      async: false,
    });
  ...
  $("#getStreetButton").click(function () {
    ...
    var response = $.ajax({
      url: `api/postalCodes.php?type=street&city_id=${cityID}&street=${street}`,
      dataType: "json",
      async: false,
    });
    json = JSON.parse(response.responseText);
  ...
  $("#getZipCodeButton").click(function () {
    ...
    var response = $.ajax({
      url: `
        api/postalCodes.php?type=zipCode&city_id=${cityID}&street_id=${streetID}&street_number=${streetNumber}`,
      dataType: "json",
      async: false,
    });
    json = JSON.parse(response.responseText);
  ...
  $("#getOffersButton").click(function () {
    ...
    var response = $.ajax({
      url: `
        api/postalCodes.php?type=offers&city_name=${cityName}&street_name=${streetName}&street_number=${streetNumber}&appartment_number=${lokalNumber}&street_id=${streetID}&city_id=${cityID}`,
      dataType: "json",
      async: false,
    });
    json = JSON.parse(response.responseText);
